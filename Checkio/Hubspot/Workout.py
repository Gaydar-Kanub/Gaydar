...
def workout(sessions: List[int], additional: int) -> int:
    deltas = []
    current_elem = sessions[0]
    for next_elem in sessions[1:]:
        deltas.append(next_elem - current_elem)
        current_elem = next_elem
    elem_dict = {}
    for elem in deltas:
        elem_dict[elem] = []
        for i in range(additional + 1):
            if elem % (i + 1) == 0:
                elem_dict[elem].append((i, int(elem / (i + 1))))
            else:
                elem_dict[elem].append((i, int(elem / (i + 1)) + 1))
